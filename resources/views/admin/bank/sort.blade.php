...
<div class="sort">
    <form method="post" action="{{ route('admin.bank.order') }}" class="cmxform form-horizontal form-sort">
        {{ csrf_field() }}
        <div class="sort_center" >
            <div class="sort_option">
                <select multiple class="form-control select">
                    @foreach($datas as $data)
                        <option class="sortids" title="{{ $data['name'] }}" value="{{ $data['id'] }}">{{ $data['name'] }}</option>
                    @endforeach
                </select>
            </div>
            <div class="sort_btn">
