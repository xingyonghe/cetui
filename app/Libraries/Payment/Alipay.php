...
class Alipay{
    ...
    protected function getConfig(){
        return config('pay.driver.alipay');
    }
     ...
     * @param array $data
     */
    public function alipaySubmit(array $data){
        //构造要请求的参数数组，无需改动
        $parameter = [
...
//    }
    ...
    public function verifyNotify($return){
        if(empty($return)) {//判断POST来的数组是否为空
            return false;
        }else {
            //生成签名结果
            $isSign = $this->getSignVeryfy($return, $return["sign"]);
            //获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
            $responseTxt = 'false';
            if (! empty($return["notify_id"])) {
                $responseTxt = $this->getResponse($return["notify_id"]);
            }
            //验证
     ...
     * @return 验证结果
     */
    public function verifyReturn($return){
        if(empty($return)) {//判断POST来的数组是否为空
            return false;
        }else {
            //生成签名结果
     ...
     * @return 提交表单HTML文本
     */
    protected function buildRequestForm($para_temp, $method, $button_name) {
        //待请求参数数组
        $para = $this->buildRequestPara($para_temp);

        $sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='".$this->alipay_gateway_new."_input_charset=".trim(strtolower($this->alipay_config['input_charset']))."' method='".$method."'>".PHP_EOL;
